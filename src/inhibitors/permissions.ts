...
import { Permission, Permissions } from "https://deno.land/x/discordeno@v7.0.0/src/types/permission.ts"
import { sendResponse } from "../utils/helpers.ts"
import { hasChannelPermission } from "https://deno.land/x/discordeno@v7.0.0/src/handlers/channel.ts"
...
botCache.inhibitors.set("permissions", async function (message, command, guild) {
  ...
  if (command.userChannelPermissions?.length) {
    const missingPermissions = command.userChannelPermissions.filter(
      (perm) => !hasChannelPermission(message.channel, message.author.id, [Permissions[perm]])
    )
