import { MessageContent } from "https://deno.land/x/discordeno@v7.0.0/src/types/channel.ts"
import { sendMessage } from "https://deno.land/x/discordeno@v7.0.0/src/handlers/channel.ts"
import { deleteMessage } from "https://deno.land/x/discordeno@v7.0.0/src/handlers/message.ts"
...
export function sendResponse(message: Message, content: string | MessageContent) {
  ...
  const contentWithMention =
    typeof content === "string" ? `${mention}, ${content}` : { ...content, content: `${mention}, ${content.content}` }
...
export function createCommandAliases(commandName: string, aliases: string | string[]) {
